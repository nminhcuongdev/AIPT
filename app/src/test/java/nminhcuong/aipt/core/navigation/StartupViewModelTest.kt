...
class StartupViewModelTest {
    ...
    fun `routes to profile flow when profile is missing`() = runTest {
        val profile = MutableStateFlow<nminhcuong.aipt.feature.profile.domain.model.UserProfile?>(null)
    ...
    fun `routes to dashboard when profile exists`() = runTest {
        val profile = MutableStateFlow<nminhcuong.aipt.feature.profile.domain.model.UserProfile?>(testUserProfile())
        val viewModel = StartupViewModel(mockProfileRepository(profile))
        backgroundScope.launch(UnconfinedTestDispatcher(testScheduler)) { viewModel.destination.collect {} }
        runCurrent()
    ...
    private fun mockProfileRepository(profile: MutableStateFlow<nminhcuong.aipt.feature.profile.domain.model.UserProfile?>): ProfileRepository {
